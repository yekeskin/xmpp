...
do_encode({teknorota_groupchat_group_created, _} =
    ...
    encode_teknorota_groupchat_group_created(Group_created,
					     TopXMLNS);
do_encode({teknorota_groupchat_update_group, _, _, _,
	   _} =
	      Update_group,
	  TopXMLNS) ->
...
do_encode({teknorota_groupchat_left_group, _} =
    ...
    encode_teknorota_groupchat_left_group(Left_group,
					  TopXMLNS);
do_encode({teknorota_groupchat_group_info, _, _, _, _,
	   _} =
	      Group_info,
	  TopXMLNS) ->
...
do_encode({teknorota_groupchat_groups_list, _} =
    ...
    encode_teknorota_groupchat_groups_list(Groups_list,
					   TopXMLNS);
do_encode({teknorota_groupchat_query, _, _, _, _, _, _,
	   _} =
	      Query,
	  TopXMLNS) ->
...
do_get_name({teknorota_groupchat_new_group, _, _, _}) ->
    <<"new-group">>;
do_get_name({teknorota_groupchat_participant, _, _,
	     _}) ->
    <<"participant">>;
do_get_name({teknorota_groupchat_query, _, _, _, _, _,
	     _, _}) ->
    <<"query">>;
...
do_get_ns({teknorota_groupchat_participant, _, _, _}) ->
    <<"teknorota:xmpp:groupchat">>;
do_get_ns({teknorota_groupchat_query, _, _, _, _, _, _,
	   _}) ->
    <<"teknorota:xmpp:groupchat">>;
...
decode_teknorota_groupchat_query(__TopXMLNS, __Opts,
    ...
    {Leave_group, Update_group, New_group, Update_keys,
     Groups_list, Group_key, Group_info} =
	decode_teknorota_groupchat_query_els(__TopXMLNS, __Opts,
					     _els, [], [], [], [], [], [], []),
    {teknorota_groupchat_query, New_group, Update_group,
     Update_keys, Leave_group, Group_info, Group_key,
     Groups_list}.
...
decode_teknorota_groupchat_query_els(__TopXMLNS, __Opts,
    ...
    {lists:reverse(Leave_group),
     lists:reverse(Update_group), lists:reverse(New_group),
     lists:reverse(Update_keys), lists:reverse(Groups_list),
     lists:reverse(Group_key), lists:reverse(Group_info)};
decode_teknorota_groupchat_query_els(__TopXMLNS, __Opts,
	...
	of
      <<"teknorota:xmpp:groupchat">> ->
	  decode_teknorota_groupchat_query_els(__TopXMLNS, __Opts,
					       _els, Leave_group, Update_group,
					       [decode_teknorota_groupchat_new_group(<<"teknorota:xmpp:groupchat">>,
										     __Opts,
										     _el)
						| New_group],
					       Update_keys, Groups_list,
					       Group_key, Group_info);
      _ ->
	  decode_teknorota_groupchat_query_els(__TopXMLNS, __Opts,
					       _els, Leave_group, Update_group,
					       New_group, Update_keys,
					       Groups_list, Group_key,
					       Group_info)
    end;
decode_teknorota_groupchat_query_els(__TopXMLNS, __Opts,
...
decode_teknorota_groupchat_query_els(__TopXMLNS, __Opts,
				     [{xmlel, <<"update-group-keys">>, _attrs,
				       _} =
					  _el
				      | _els],
				     Leave_group, Update_group, New_group,
...
encode_teknorota_groupchat_query({teknorota_groupchat_query,
				  New_group, Update_group, Update_keys,
				  Leave_group, Group_info, Group_key,
				  Groups_list},
				 __TopXMLNS) ->
    __NewTopXMLNS =
...
decode_teknorota_groupchat_group_info(__TopXMLNS,
	...
	decode_teknorota_groupchat_group_info_attrs(__TopXMLNS,
						    ...
						    undefined, undefined,
						    undefined),
    {teknorota_groupchat_group_info, Id, Name, Avatar,
...
decode_teknorota_groupchat_group_info_attrs(__TopXMLNS,
    ...
    decode_teknorota_groupchat_group_info_attrs(__TopXMLNS,
						_attrs, _val, Name, Avatar,
						Lastupdated);
decode_teknorota_groupchat_group_info_attrs(__TopXMLNS,
					    [{<<"name">>, _val} | _attrs], Id,
					    _Name, Avatar, Lastupdated) ->
    decode_teknorota_groupchat_group_info_attrs(__TopXMLNS,
						_attrs, Id, _val, Avatar,
						Lastupdated);
decode_teknorota_groupchat_group_info_attrs(__TopXMLNS,
...
decode_teknorota_groupchat_group_info_attrs(__TopXMLNS,
    ...
    decode_teknorota_groupchat_group_info_attrs(__TopXMLNS,
						_attrs, Id, Name, Avatar, _val);
decode_teknorota_groupchat_group_info_attrs(__TopXMLNS,
					    [_ | _attrs], Id, Name, Avatar,
					    Lastupdated) ->
    decode_teknorota_groupchat_group_info_attrs(__TopXMLNS,
						_attrs, Id, Name, Avatar,
						Lastupdated);
decode_teknorota_groupchat_group_info_attrs(__TopXMLNS,
					    [], Id, Name, Avatar,
					    Lastupdated) ->
...
decode_teknorota_groupchat_update_group(__TopXMLNS,
	...
	decode_teknorota_groupchat_update_group_attrs(__TopXMLNS,
						      _attrs, undefined,
						      undefined, undefined),
    {teknorota_groupchat_update_group, Id, Name, Avatar,
     Participants}.
...
decode_teknorota_groupchat_new_group(__TopXMLNS, __Opts,
	...
	decode_teknorota_groupchat_new_group_attrs(__TopXMLNS,
						   _attrs, undefined,
						   undefined),
    {teknorota_groupchat_new_group, Name, Avatar,
     Participants}.
...
decode_teknorota_groupchat_participant_attrs(__TopXMLNS,
					     ...
					     Type, Action) ->
    decode_teknorota_groupchat_participant_attrs(__TopXMLNS,
						 _attrs, _val, Type, Action);
decode_teknorota_groupchat_participant_attrs(__TopXMLNS,
					     [{<<"type">>, _val} | _attrs], Jid,
					     _Type, Action) ->
    decode_teknorota_groupchat_participant_attrs(__TopXMLNS,
						 _attrs, Jid, _val, Action);
decode_teknorota_groupchat_participant_attrs(__TopXMLNS,
					     [{<<"action">>, _val} | _attrs],
					     Jid, Type, _Action) ->
    decode_teknorota_groupchat_participant_attrs(__TopXMLNS,
						 _attrs, Jid, Type, _val);
decode_teknorota_groupchat_participant_attrs(__TopXMLNS,
					     [_ | _attrs], Jid, Type, Action) ->
    decode_teknorota_groupchat_participant_attrs(__TopXMLNS,
						 _attrs, Jid, Type, Action);
decode_teknorota_groupchat_participant_attrs(__TopXMLNS,
					     [], Jid, Type, Action) ->
